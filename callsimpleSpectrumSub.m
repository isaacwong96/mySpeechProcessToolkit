...
N=length(x);                            % 信号长度
time=(0:N-1)/fs;                        % 设置时间
signal=gaussNoiseGen(x,SNR);                % 叠加噪声
snr1=calSNR_singleCh(x,signal);            % 计算初始信噪比
overlap=wlen-inc;                       % 求重叠区长度
NIS=fix((IS*fs-wlen)/inc +1);           % 求前导无话段帧数

a=4; b=0.001;                           % 设置参数a和b
output=simpleSpectrumSub(signal,wlen,inc,NIS,a,b);% 谱减
snr2=calSNR_singleCh(x,output);            % 计算谱减后的信噪比
snr=snr2-snr1;
